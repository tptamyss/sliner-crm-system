...
def setup_google_calendar_auth():
    ...
    if os.path.exists('token.json'):
        ...
        **To connect Google Calendar:**
        ...
        if st.button("🔗 Connect Google Calendar") and os.path.exists('credentials.json'):
            try:
                ...
                st.write(f"[Authorize Google Calendar]({auth_url})")
                
                # Get authorization code from user
                auth_code = st.text_input("2. Paste the authorization code here:")
                
                if auth_code and st.button("Complete Setup"):
...
def add_meeting(customer_id, title, datetime_obj, description, created_by, auto_approve=False):
    ...
    if approved:
        service = get_google_calendar_service()
        if service:
            # Get customer and user info for calendar event
            cursor.execute('''
                SELECT c.name, c.email, c.representative, u.name, u.email
                FROM customers c
            ...
            if customer_info:
                customer_name, customer_email, customer_rep, user_name, user_email = customer_info
                
                calendar_title = f"Meeting: {title}"
...
Description: {description}
                ...
                success, result = create_google_calendar_event(
                    calendar_title, 
                    datetime_obj, 
                    None, 
                ...
                if success:
                    ...
                    cal_notification_id = str(uuid.uuid4())
                    cursor.execute('''
                        INSERT INTO notifications (id, user_id, message, type)
                        VALUES (?, ?, ?, ?)
                    ''', (cal_notification_id, created_by, f"Meeting added to Google Calendar: {title}", "calendar_created"))
    ...
    # Create notification for assigned employee about the meeting
    if approved:
        # Get customer info to find assigned employee
...
def approve_meeting(meeting_id):
    ...
    # Add to Google Calendar when approved
    service = get_google_calendar_service()
    if service and meeting_info:
        cursor.execute('''
            SELECT m.title, m.datetime, m.description, c.name, c.email, c.representative, u.name, u.email
            FROM meetings m
            JOIN customers c ON m.customer_id = c.id
            JOIN users u ON c.assigned_to = u.id
            WHERE m.id = ?
        ...
        if full_info:
            m_title, m_datetime, m_desc, c_name, c_email, c_rep, u_name, u_email = full_info
            
            calendar_title = f"Meeting: {m_title}"
            calendar_desc = f"Meeting with {c_name}\nRepresentative: {c_rep}\n\n{m_desc}"
            
            attendees = [u_email]
            if c_email:
                attendees.append(c_email)
            
            # Convert string datetime back to datetime object
            meeting_dt = datetime.fromisoformat(m_datetime.replace('Z', ''))
            
            success, result = create_google_calendar_event(
                calendar_title, meeting_dt, None, calendar_desc, attendees
            )
